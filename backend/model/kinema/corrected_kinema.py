...
class CorrectedKinema:
    ...
    def forward(self, joint: np.ndarray) -> np.ndarray:
        """1 組の関節角から、たわみを加えた幾何パラメータで手先姿勢を計算する。"""
        delta1, delta2, delta3, delta4, delta5, delta6, phi2, w2, phi4, w4, psi1 = self.calc_deform(joint)

        # 関節角（J4, J6 は向きが逆）を W 回転に加え、たわみによる角度・変位を足し込む
        parameters = self.params.geometry_param_corr.copy()
        parameters[:6, 5] += joint * np.array([1, 1, 1, -1, 1, -1], dtype=np.float64)
        parameters[:, 3:] = np.radians(parameters[:, 3:])
        parameters += np.array([
            [0, 0, 0, 0, 0, delta1], [0, 0, 0, 0, 0, delta2],
            [0, w2, 0, 0, 0, delta3 + phi2], [w4, 0, 0, 0, phi4, delta4],
            [0, 0, 0, 0, 0, delta5], [0, 0, 0, 0, 0, delta6], [0, 0, 0, 0, 0, 0],
        ], dtype=np.float64)

        # 各リンクの XYZ 並進 + UVW 回転を順に掛ける（ベースの倒れ psi1 は第 1 リンクの後に挟む）
        transform = np.eye(4, dtype=np.float64)
        for index in range(7):
            u, v, w = parameters[index, 3:]
            cu, su, cv, sv, cw, sw = np.cos(u), np.sin(u), np.cos(v), np.sin(v), np.cos(w), np.sin(w)
            transform = transform @ np.array([
                [cv * cw, -cv * sw, sv, parameters[index, 0]],
                [su * sv * cw + cu * sw, -su * sv * sw + cu * cw, -su * cv, parameters[index, 1]],
                [-cu * sv * cw + su * sw, cu * sv * sw + su * cw, cu * cv, parameters[index, 2]],
                [0, 0, 0, 1],
            ], dtype=np.float64)
            if index == 0:
                cosine, sine = np.cos(psi1), np.sin(psi1)
                transform = transform @ np.array([[cosine, 0, sine, 0], [0, 1, 0, 0], [-sine, 0, cosine, 0], [0, 0, 0, 1]], dtype=np.float64)
    ...
    def calc_deform(self, joint: np.ndarray) -> tuple[float, ...]:
        """重力荷重に起因する関節角・アームのたわみを算出する。"""
        ...
        th1, th2, th3, th4, th5, th6 = np.radians(joint)
        ...
        gravity = GRAVITY * self.params.gravity_direction @ np.array([[c1, -s1, 0], [s1, c1, 0], [0, 0, 1]])
        mass = self.params.mass
        moment5_vector = np.array([0, m5 * lg7, 0]) + mass * np.array([lgty * c6 - lgtx * s6, l6 + lgtz, lgty * s6 + lgtx * c6])
        moment5 = np.cross(rotation5 @ moment5_vector, gravity)
        moment3_vector = np.array([0, m3 * lg6 + m5 * l5, m3 * lg5 + m5 * l4]) + mass * np.array([0, l5, l4])
        moment3 = moment5 + np.cross(np.array([0, c23 * moment3_vector[1] - s23 * moment3_vector[2], s23 * moment3_vector[1] + c23 * moment3_vector[2]]), gravity)
        moment2_vector = np.array([0, m2 * lg4, m2 * lg3 + (m3 + m5) * l3]) + mass * np.array([0, 0, l3])
        moment2 = moment3 + np.cross(np.array([0, c2 * moment2_vector[1] - s2 * moment2_vector[2], s2 * moment2_vector[1] + c2 * moment2_vector[2]]), gravity)
        ...
        joint_moments = np.array([
            moment1[2], moment2[0], moment3[0], moment5[1] * c23 + moment5[2] * s23,
            moment5[0] * rotation5[0, 0] + moment5[1] * rotation5[1, 0] + moment5[2] * rotation5[2, 0],
            moment5[0] * rotation5[0, 1] + moment5[1] * rotation5[1, 1] + moment5[2] * rotation5[2, 1],
        ])
        ...
        return (*deltas, phi2, w2, phi4, w4, psi1)
