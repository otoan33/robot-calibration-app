...
| | ツール補正 | 工具オフセットを推定する |
| 逐次最適化 | – | ロボットに動かして測り、パラメータ（足りなければ動作軌道も）を繰り返し直して目標軌跡に近づける |
...
- 画面上部の **「描画」「解析」「逐次最適化」** で切り替える
- 描画：CSV からグラフを作る／解析：補正パラメータを推定する
- 逐次最適化：ロボットに動かしながら補正を繰り返す
...
![bg right:64% contain](img/34_opt_settings.png)

- 動かす → 測る → パラメータを直す、を繰り返して目標の軌跡に近づける
- **calibration_mode**・**同定パターン**：何を推定するか（既定は剛性を推定しない `kinema_actual`）
- **目標精度**・**最大反復回数** で終わり、改善が **改善率** を下回ると軌道修正に移る。**ゲイン** は軌道を直す強さ

---

## 35. 目標の軌道を入れて開始する

![bg right:64% contain](img/35_opt_start.png)

- 目標の関節角軌道 `*_FM.csv` を 1 つ入れる（入れなければデモ軌道）
- **開始** で最後まで自動で繰り返す。「1 反復」なら 1 回ずつ進む
- **停止** は、今の反復が終わってから止まる
...
![bg right:64% contain](img/36_opt_convergence.png)

- **グラフ**：反復ごとの目標軌跡との誤差（RMS・最大）。点線が目標精度
- 網掛けの範囲は軌道修正の段階
- **上の表示** で、何反復済みかと次の段階が分かる

---

## 37. 軌跡と誤差の時系列を見る

![bg right:64% contain](img/37_opt_trajectory.png)

- **3D**：目標（黒）と、初回・最新の計測。誤差は「3D の誤差表示倍率」倍に拡大
- **時系列**：初回・前回・最新の誤差の大きさ [mm]
- 3D はドラッグで回せる
...
![bg right:64% contain](img/38_opt_parameters.png)

- **パラメータの推移**：軸ごとの伝達誤差の振幅と、前回からの幾何パラメータの変化
- 変化が小さくなれば、推定が落ち着いている
- **表**：反復ごとの段階・RMS・最大・所要時間

---

## 39. 履歴を保存する・やり直す

![bg right:64% contain](img/39_opt_save.png)

- **履歴を保存**：各反復の誤差と推定パラメータ、最後の指令軌道を JSON で保存
- **リセット**：履歴を消して最初からやり直す
...
![bg right:64% contain](img/40_error.png)
...
| 逐次最適化 | `*_FM.csv` 1 つ（目標） | `Time[ms]`, `Joint(J1〜J6)[deg]` |
